...
class AsyncGenTCReq : public AsyncTexRequest {
 public:
  ...
  virtual void Preload(const std::string &fname) {
    ...
    void *buf_mem = clEnqueueMapBuffer(_ctx->GetCommandQueue(), _cmp_buf,
    ...
    CHECK_CL(clEnqueueUnmapMemObject, _ctx->GetCommandQueue(), _cmp_buf,
  ...
  virtual std::vector<cl_event> QueueDXT() {
    ...
    CHECK_CL(clEnqueueMarkerWithWaitList, _ctx->GetCommandQueue(), 2, wait_events, &init_event);
